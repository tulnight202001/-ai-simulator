...
## Unreleased — 任務資訊與 Agent 並行對齊

### 已完成

- 十一種工作區已加入機台本體上的大型功能圖示、明顯主色與不同輪廓；訂單卡與資料夾共用同一圖示語彙。
- 頂部訂單卡已可點開完整內容，顯示目的、動作、成果、固定／自由工序、全部步驟與完成狀態。
- 配方已區分 `ordered` 與 `flexible`；自由工序完成狀態依 step index，錯機台維持完全靜默且不提供正確答案。
- 五種接單選項已具不同玩法結果並在選擇前預覽：接受原單、追問規格、限制範圍、替代路線與拒絕各自影響耐心、工序、品質、報酬或滿意度。
- Agent 已改為場景中可見、抱資料夾跟隨並接受玩家機台指派的虛擬機器人；完成後會抱回資料夾。
- Era 4 已限制最多兩台、Era 5 最多三台機器獨立並行；單一資料夾仍只能在一台機器加工，且開始後不可中斷或改派。
- 訂單卡已顯示玩家、Agent 與機台加工狀態；Era 4／5 試玩複本會提供相應 Agent 與並行上限。
- 第一版持續以手機 9:16 直向優先；電腦共用畫面與規則。

### 驗證

- `npm test` 27/27、`npm run check`、`npm run build` 及 390×844 首頁／選角／Era 5 冒煙通過；建置僅有既存 Phaser chunk size warning。
- 尚待推送公開版後由使用者以 iPhone 真機驗收。
