...
- `src/data/artCatalog.ts` 是正式美術的唯一入口；客戶與全部 11 種工作站已切到 `public/art/generated/v3/`，AI 與背景仍使用 v2。
- 排隊、個別耐心、接單、交付與追加規則在 `src/core/v1Runtime.ts`；關卡客流與產品目的／動作／成果在 `src/data/v1Catalog.ts`；訂單卡、客戶隊列、資料夾、工作站與交付盤呈現在 `src/game/WorkstationScene.ts`。
- `src/main.ts` 的試玩模式可直接進入 Era 1–5 第一關。它只複製目前生涯供場景計算，結算不得呼叫正式進度或存檔寫入。
- 六位客戶採程式 tween 的輕微待機動態，目前不是逐幀角色動畫。後續優先做手機真機觸控／效能與高峰客流平衡，不要再把客戶移到櫃檯前，也不要把 AI 換成人類角色。
- GitHub Pages 由 `.github/workflows/docs-pages.yml` 發布；推送 `full-v1-development` 後會先執行 install、test、check、build。v3 PNG 必須保持在 Git 追蹤範圍，否則乾淨建置會出現客戶圖 404。

### 本批接手注意事項

- 首頁／選角 AI 尺寸已按手機直向校準；開發模式會解除舊 service worker 控制並重載，正式 service worker 已升到 `ai-simulator-v5-20260730` 且採 network-first，勿恢復成舊版 cache-first。
- `StepIconView` 是訂單卡與資料夾共用的彩色幾何圖示來源。錯機台必須保持完全靜默且無扣分；未完成訂單不得顯示下一站文字答案。
- 五紀元首關試玩只使用生涯複本，結算不得寫入正式進度。390×844 視覺檢查已通過，下一輪以公開版 iPhone 真機回饋為準。
- 本批最終基準為 `npm test` 22/22、check 通過、build 通過；唯一訊息是既有 Phaser chunk size warning。

## 2026-07-30 最新玩法完成狀態與接手邊界

- 第一版正式介面仍是 9:16 手機直向；電腦共用同一場景與規則。新增任何資訊面板或 Agent 控制時，先確保手機文字、觸控熱區、安全區與主要動線，再補鍵盤／滑鼠別名。
- 十一種工作區已各有機台本體上的大型功能圖示、明顯主色與不同輪廓；訂單卡、客戶資料夾、玩家資料夾和工作區共用同一套圖示映射。後續不可退回只靠小字或相似機台外觀。
- 訂單卡已可點開完整目的、動作、成果、`ordered`／`flexible`、全部步驟圖示、完成狀態、耐心及已知報酬／品質；展開內容不會額外顯示正確下一站。
- `src/data/v1Catalog.ts` 是配方順序模式來源；`src/core/v1Runtime.ts` 負責固定／自由步驟與五種接單決策。自由工序的畫面勾選已改依 step index。
- 五種接單決策已在選擇前預覽實際取捨，不再只是不同文案；其耐心、工序、品質、報酬及滿意度結果仍由核心規則控制。
- Agent 已是場景中可見的虛擬機器人，能抱自己的資料夾跟隨；玩家可交箱、選機台及指派，Agent 完成後會抱回資料夾，不再以接單後不可見自動代做作為主要互動。
- 多工已改為每台機器的獨立工作狀態。Era 4 上限兩台、Era 5 上限三台；訂單卡會顯示玩家／Agent／機台狀態，單一資料夾同時只在一台機器，開始後不能中斷或改派。
- 錯機台對玩家與 Agent 都完全靜默：不顯示正確站、不閃提示、不震動、不播放錯誤音效、不扣分、不改資源。核心內部診斷原因不得轉呈正式 UI。
- Era 4／5 首關試玩會在不保存的生涯複本內提供相應 Agent 等級與兩／三台並行上限；結算仍不寫入正式進度。
- 最新基準：`npm test` 27/27、`npm run check` 通過、`npm run build` 通過，僅有既存 Phaser chunk size warning；390×844 首頁、選角及 Era 5 完整玩法冒煙通過。
- 尚待推送公開版後由使用者以 iPhone 真機驗收。後續若無實際問題，不重做相同本機驗證。
